...
class UserInfoManager //pointer to user info linkedlist
{
    ...
    public:
        ...
        static UserInfo*& getMyList() {
            static UserInfo* mylist = nullptr; // This will only be initialized once.
            return mylist;
        }

        UserInfoManager()
        {
            UserInfo*& myList = getMyList();
            myList = NULL;
...
void UserInfoManager::deleteUser(string username)
{
    ...
    if (myList->name == username) {
        UserInfo* toDelete = myList;
        myList = myList->next; // Point head to the next element
...
void UserInfoManager::massLoadAndCompute(string filename){
    ...
    if(!myFile){
        ...
        while (getline(File, line)){
                ...
                //adding or readding values for carbs protein fat calories
                ...
                if (new_user->gender == "male"){
                    if (new_user->age >= 19 && new_user->age <= 30){
                        if (new_user->lifestyle == "sedentary"){
                                result = 2400;
                        }
                        else if (new_user->lifestyle == "moderate"){
                                result = 2600;
                        }
                        else {
                                result = 3000;
                    ...
                    else if (new_user->age >= 31 && new_user->age <= 50){
                        if (new_user->lifestyle == "sedentary"){
                                result = 2200;
                        }
                        else if (new_user->lifestyle == "moderate"){
                        ...
                        else if (new_user->lifestyle == "mdderate"){
                                result = 2200;
                        }
                        else {
                                result = 2400;
                ...
                new_user->calories = result;

                //carbs, protein, fat
                double calories = new_user->calories;
                
                new_user->carbs = (calories * 0.5) / 4;

                new_user->protein = (calories * 0.3) / 4;

                new_user->fat = (calories * 0.2) / 9;
                ...
                string group;
                if (new_user->gender == "female"){
                    BF_percentage = 495 / ( 1.29579 - 0.35004 * log10( new_user->waist + new_user->hip - new_user->neck ) + 0.22100 * log10( new_user->height ) ) - 450;
                    int bf_perc = BF_percentage;
                    cout << "bf_percentage: " << BF_percentage << endl;
                    bool wrong = false; //if bf_percentage is Nan


                    if (new_user->age >= 20 && new_user->age <= 39){
                        if (BF_percentage < 21) res = "(" + std::to_string(bf_perc) + " . low)";
                        else if (BF_percentage >= 21 && BF_percentage < 33.0) res = "(" + std::to_string(bf_perc) + " . normal)";
                        else if (BF_percentage >= 33 && BF_percentage < 39.0) res = "(" + std::to_string(bf_perc) + " . high)";
                        else if (BF_percentage >= 39) res = "(" + std::to_string(bf_perc) + " . high)";
                        else {
                                cout << "bf_percentage is Nan"<<endl;
                                bool wrong = true;
                        }
                    }
                    else if (new_user->age >= 40 && new_user->age <= 59){
                        if (BF_percentage < 23) res = "(" + std::to_string(bf_perc) + " . low)";
                        else if (BF_percentage >= 23 && BF_percentage < 34.0) res = "(" + std::to_string(bf_perc) + " . normal)";
                        else if (BF_percentage >= 34 && BF_percentage < 40.0) res = "(" + std::to_string(bf_perc) + " . high)";
                        else if (BF_percentage >= 40) res = "(" + std::to_string(bf_perc) + " . high)";
                        else {
                                cout << "bf_percentage is Nan"<<endl;
                                bool wrong = true;
                        }
                    }
                    else if (new_user->age >= 60 && new_user->age <= 79){
                        if (BF_percentage < 24) res = "(" + std::to_string(bf_perc) + " . low)";
                        else if (BF_percentage >= 24 && BF_percentage < 36.0) res = "(" + std::to_string(bf_perc) + " . normal)";
                        else if (BF_percentage >= 36 && BF_percentage < 42.0) res = "(" + std::to_string(bf_perc) + " . high)";
                        else if (BF_percentage >= 42) res = "(" + std::to_string(bf_perc) + " . high)";
                        else {
                                cout << "bf_percentage is Nan"<<endl;
                                bool wrong = true;
                        }
                ...
                // for males
                if (new_user->gender == "male"){
                    BF_percentage = 495 / ( 1.0324 - 0.19077 * log10( new_user->waist - new_user->neck ) + 0.15456 * log10( new_user->height ) ) - 450;
                    int bf_perc = BF_percentage;
                    cout << "bf_percentage: " << BF_percentage << endl;
                    bool wrong = false; //if bf_percentage is Nan
                    ...
                    if (new_user->age >= 20 && new_user->age <= 39){
                        if (BF_percentage < 8) 
                        {    
                                res = "(" + std::to_string(bf_perc) + " . low)";
                        }
                        else if (BF_percentage >= 8 && BF_percentage < 20.0) 
                        {
                                res = "(" + std::to_string(bf_perc) + " . normal)";
                        }
                        else if (BF_percentage >= 20 && BF_percentage < 25.0)
                        {
                                res = "(" + std::to_string(bf_perc) + " . high)";
                        }
...
{
    UserInfo*& myList = getMyList();
    UserInfo* temp = myList;  //correct this too
    double result;
    bool found = false;
...
void HealthAssistant::massLoadAndCompute(string filename){
    UserInfoManager::massLoadAndCompute(filename);
} // wrapper method
